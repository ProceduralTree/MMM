...
def assemble_matrix_2d(h , N,M):
    ...
    x = np.arange(0,N)* h
    y = np.arange(0,M)* h
    halfgrid_x = np.meshgrid(x_h,y)
    halfgrid_y = np.meshgrid(x,y_h)
    half_diffusion_x = D(halfgrid_x[0] , halfgrid_x[1])
    half_diffusion_y = D(halfgrid_y[0] , halfgrid_y[1])
    main_diag[1:-1,1:-1] =  half_diffusion_x[:-1,:] + half_diffusion_x[1:,:] + half_diffusion_y[:,:-1] + half_diffusion_y[:,1:]
    main_diag = np.ravel(D)
